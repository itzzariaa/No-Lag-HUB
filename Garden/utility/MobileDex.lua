...
local function main()
  ...
  Explorer.ShowRightClick = function(MousePos)
    ...
    if presentClasses["LuaSourceContainer"] then
      context:AddRegistered("SAVEFILE_SCRIPT")
  ...
  Explorer.InitRightClick = function()
    ...
    context:Register("SAVEFILE_SCRIPT",{Name = "Save File Script", IconMap = Explorer.MiscIcons, Icon = "ViewScript", OnClick = function()
      for _, v in next, selection.List do
        if v.Obj:IsA("LuaSourceContainer") then
          local success, decompiled = pcall(decompile, v.Obj)
          if success then
            writefile(("%i.Script.%s.txt"):format(game.PlaceId, v.Obj.Name), decompiled)
            task.wait(0.2)
          end
        end
      end
...
local function main()
  ...
  ScriptViewer.SaveFileScript = function(scr)
    local success, source = pcall(decompile or function() end, scr)
    if not success or not source then source, PreviousScr = "-- DEX - Source failed to decompile", nil else PreviousScr = scr end

    env.writefile("Name_"..scr.."_Script_"..os.time()..".txt", source)
  end
...
Main = (function()
  ...
  Main.InitEnv = function()
        ...
        env.decompile = decompile or (env.getscriptbytecode and env.request and env.base64encode and function(scr)end)
  ...
  Main.CreateMainGui = function()
    ...
    Main.CreateApp({Name = "Click Gui Objects to select", IconMap = Main.LargeIcons, Icon = 6, OnClick = function(callback)
      if callback then
        local mouse = Main.Mouse
        local player = service.Players.LocalPlayer
